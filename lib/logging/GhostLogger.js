...
GhostLogger.prototype.setSyslogStream = function () {
    var Bunyan2Syslog = require('bunyan-syslog');

    var syslogStream = Bunyan2Syslog.createBunyanStream({
			type: 'sys',
			facility: Bunyan2Syslog.local0,
			host: this.syslog.host,
			port: this.syslog.port
    });

    this.streams.syslog = {
        name: 'syslog',
        match: this.syslog.match,
        log: bunyan.createLogger({
            name: this.syslog.name,
            streams: [{
                type: 'raw',
                stream: syslogStream,
                level: this.level
